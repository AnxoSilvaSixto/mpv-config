...
The shortcut to adjust timing is set to ctrl+/ by default.
...
works best). Then hit ctrl+/ to mark the first point. Go on to a later time
where someone speaks, and synchronize the right subtitle again. Then hit ctrl+/
...
-- input.conf owns Ctrl+/ (script-binding media/sub-set-time); nil key avoids a double-binding.
