--[[
    This script calls a PowerShell helper (Set-RefreshRate.ps1, using the real Win32 display API directly —
    no third-party tool) to change the refresh rate of the display the mpv window is currently open in
    ...
    These script messages completely bypass the whitelist and rate associations and are sent to Set-RefreshRate.ps1 directly, so make sure you send a valid integer.
...
local options = {
    ...
    --Set-RefreshRate.ps1 only accepts integers (same constraint nircmd had), DO NOT use the full refresh rate, i.e. 23.976
    ...
    --colour bit depth to send to Set-RefreshRate.ps1
...
--calls the PowerShell helper to change the display resolution and rate
function changeRefresh(width, height, rate, display)
    ...
    --Added 2026-08-23: skip the whole display-mode change (subprocess spawn + Win32 call) when
    --already at the target mode, not just the pause below. Previously only the pause was skipped,
    --so back-to-back same-framerate episodes still spawned powershell.exe and called
    ...
    msg.verbose('calling powershell with: ' .. options.helper_script .. " -Width " .. width .. " -Height " .. height .. " -Rate " .. rate .. " -DeviceName " .. var.dname)

    msg.info("changing display " .. var.dname .. " to " .. width .. "x" .. height .. " " .. rate .. "Hz")

    --pauses the video while the change occurs to avoid A/V desyncs
    if options.pause > 0 and not mp.get_property_bool("pause") then
    ...
    local process = mp.command_native({
        ...
        args = {
            ...
            "-File", options.helper_script,
            "-Width", width,
            "-Height", height,
            "-Rate", rate,
            "-DeviceName", var.dname,
            "-BitDepth", options.bdepth
        },
        capture_stdout = true,
        capture_stderr = true
    })
    ...
    if (process.status ~= 0) then
        ...
        if process.error_string == "init" then
            msg.error('could not start powershell.exe')
        else
...
--(historical: nircmd itself isn't called anymore -- see changeRefresh below, which passes
--Set-RefreshRate.ps1 the string name directly. This number now only serves this script's own
--0-based script-message API and OSD text.)
...
function getDisplayDetails()
    ...
    --extract the display number from \\.\DISPLAY# (handles DISPLAY10+ correctly)
    local numberMatch = name:match('DISPLAY(%d+)$')
    local number = numberMatch and (tonumber(numberMatch) - 1) or 0
