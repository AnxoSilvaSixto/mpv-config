...
- `settings.xml:2` selects `x86_64-v3` (AVX2) — Zen 3 5700X assumption. Revert to `x86_64` for non-v3 CPUs. Setting is read by `installer/updater.ps1:401` (`Check-Arch`) and hardcoded as `zhongfly` v3 pattern in `Update-MpvEnvironment.ps1:125`
...
- To customize HDR: edit `mpv.conf` profiles or the transform rules in `Update-MpvEnvironment.ps1:267-275`
- Transforms applied on sync: `~~/shaders/hdr-toys/` → `~~/shaders/hdr-toys/` (portable `~~/` sometimes fails) and `gamut-mapping/bottosson.glsl` → `gamut-mapping/jedypod.glsl` (upstream v2504 note). See `Update-MpvEnvironment.ps1:270-273`
...
- Keep portable `~~/...` in `mpv.conf`, `input.conf` and shader appends — `~~/` is documented to sometimes fail under `portable_config` (see `Update-MpvEnvironment.ps1:40,271` and `mpv.conf:34`). Only `mpv.conf:64-65` keeps `~~/` for cache dirs (the sole place it is known safe)
...
- HDR: `include="~~/hdr-toys.conf"` (`mpv.conf:34`) — loads before profiles; must stay before any `[profile]`
...
- Screenshots: `png`, `screenshot-high-bit-depth=yes`, `screenshot-tag-colorspace=yes`, to `~/Pictures/mpv` (`mpv.conf:47-50`)
...
Generated by `Update-MpvEnvironment.ps1:267-275` from upstream `natural-harmonia-gropius/hdr-toys` `hdr-toys.conf` with 2 text transforms.
...
- All keep portable `~~/...` after sync. `Alt+h` in `input.conf:63` must del **all 9** shaders across these 4 profiles to fully disable — fixed 2026-08-25 (was only 5 PQ shaders before).
...
- `uosc_icons.ttf` 1,206,668 bytes + `uosc_icons.otf` 400,360 bytes (same icon set, two formats) + `uosc_textures.ttf` 38,228 bytes. Required for uosc UI. Copied by `Update-MpvEnvironment.ps1:279-281` from `uosc/src/fonts/`.
...
- Mutex `Global\mpv-autoupdate-lock` (`Update-MpvEnvironment.ps1:252`) prevents overlapping runs — if another instance holds lock, exits immediately. Pairs with Scheduled Task `MultipleInstances IgnoreNew`
- Cleanup + log rotation at end (`Update-MpvEnvironment.ps1:288-315`): `shaders_cache >30d`, `watch_later >7d`, `update-log.txt >500 lines` (keeps last 400)
...
| Change HDR | Edit shader choice in `Update-MpvEnvironment.ps1:272` transforms or `mpv.conf` target-*; **not** `hdr-toys.conf` | Hand-edit lost on next update |
...
| Fix thumbfast not updating | Check `Update-MpvEnvironment.ps1:285` and the top-level `portable_config/scripts/thumbfast.lua` destination | Updater must refresh the script in the location mpv scans |
...
1. **v3 asset missing (2026-08-27):** `zhongfly` release `2026-08-26-182fa6ca49` shipped no `x86_64-v3` asset — updater correctly waited one day rather than downgrading to baseline. Don't add fallback to plain `x86_64` in `Update-MpvEnvironment.ps1:125` — v3-only is intentional.
...
10. **thumbfast is a top-level script:** `portable_config/scripts/thumbfast.lua` is updated directly by `Update-MpvEnvironment.ps1:285` and is discovered by mpv's recursive script loader. Keep it at the top level; do not restore a nested loader shim.
...
| uosc | [tomasklaen/uosc](https://github.com/tomasklaen/uosc) | `portable_config/scripts/uosc/` (43 KB + 40 modules) + `fonts/uosc_*` | `Update-MpvEnvironment.ps1:277` via commit SHA → codeload zip |
| thumbfast | [po5/thumbfast](https://github.com/po5/thumbfast) | `portable_config/scripts/thumbfast.lua` (32 KB) | `Update-MpvEnvironment.ps1:285` |
| hdr-toys | [natural-harmonia-gropius/hdr-toys](https://github.com/natural-harmonia-gropius/hdr-toys) | `portable_config/shaders/hdr-toys/` (77 files, 298 KB) + `hdr-toys.conf:1` | `Update-MpvEnvironment.ps1:267` with 2 transforms |
...
- Remote: `https://github.com/<user>/mpv-config.git`
...
- [ ] If edited `hdr-toys.conf` — STOP, revert, edit `mpv.conf` or `Update-MpvEnvironment.ps1:267-275` instead
