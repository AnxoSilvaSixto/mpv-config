...
│   │   ├── track-selector.lua       # commentary-safe select (Chinna95P)
│   │   ├── thumbfast.lua              # top-level mpv script named "thumbfast"
│   │   ├── uosc/                      # UI and bundled modules
│   │   ├── media/                     # 6 files: skip_intro, sub-select (DISABLED), betterchapters, fix-sub-timing, Up_Next + loader
...
- Resolution profiles select CfL, ravu, ArtCNN + SSimSuperRes/Downscaler anti-ringing; dither=fruit, fbo-format=rgba16hf. The six top-level shader files use Git LFS (ArtCNN, CfL, nlmeans, ravu + SSimSuperRes/Downscaler); `hdr-toys/` remains ordinary text.
